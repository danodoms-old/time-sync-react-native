...
export default function Schedule() {
  ...
  useEffect(() => {
    const fetchSchedules = async () => {
      try {
        const newSchedules = (await getSchedules()).schedules;
        //const newSchedules = scheduleObj.schedules

        console.log("newSchedules: ", newSchedules);
        setSchedules(newSchedules);
      } catch (error) {
        console.error("Error fetching schedules: ", error);
        setSchedules([]); // Set schedules to an empty array in case of error
      }
    };
    ...
    <View>
      ...
      >
        <TabsList className="flex-row w-full">
          {days.map((day, index) => (
            <TabsTrigger value={day} className="flex-1" key={index}>
      ...
      >
        ...
        {schedules.length > 0 ? schedules.map((day, index) => (
          <View className="p-4 w-screen gap-2" key={index}>
            <Text className="font-bold text-3xl pb-2">{days[index]}</Text>
